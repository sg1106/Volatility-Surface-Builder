...
class VolatilitySurface:
    ...
    def predict(self, K, T):
        ...
        if self._spline is None:
            return np.nan
        val = self._spline(K, T, grid=False)
    ...
    def grid(self, n_strikes=60, n_maturities=30, strike_range=0.4):
        ...
        # grid=True here is exactly what we want: K_vals/T_vals define the axes
        # of a full n_maturities x n_strikes grid in one vectorized spline call.
        IV_grid = np.clip(self._spline(K_vals, T_vals).T, 0.01, 5.0)
